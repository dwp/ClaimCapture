...
class DwpCSRFAction(next: EssentialAction,
  ...
  def apply(request: RequestHeader) = {
    ...
    if (CSRFConf.UnsafeMethods(request.method) && request.contentType.exists(CSRFConf.UnsafeContentTypes)) {
      ...
      if (checkCsrfBypass(request) || Play.isTest) {
        continue
      } else {
  ...
  private def checkFailed(msg: String): Iteratee[Array[Byte], SimpleResult] = throw new DwpRuntimeException(s"Invalid CSRF - ${msg}") //Done(Forbidden(msg))
