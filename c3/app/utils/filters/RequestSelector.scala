...
object RequestSelector {
  ...
  def toBeChecked(request:RequestHeader) = {
    ...
    !headerString.matches(".*assets.*") && !headerString.matches(".*images.*") && !endPage(request) && !headerString.matches(".*error.*") && !headerString.matches(".*back-button.*") && !headerString.matches("/report/.*")
  }
