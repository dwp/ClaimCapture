...
object Claim {
  def apply() = {
    val sections = List(
      Section(CarersAllowance, firstPage = controllers.s1_carers_allowance.routes.G1BenefitsMandatory.present(), lastPage = controllers.s1_carers_allowance.routes.CarersAllowance.approve()),
      ...
      Section(Employed, firstPage = controllers.s7_employment.routes.G2JobDetails.present(), lastPage = controllers.s7_employment.routes.Employment.completed()),
