...
object  Declaration {
  ...
  def xml(claim: Claim) = {
    ...
    <Declaration>
      ...
      <TextLine></TextLine>
      <TextLine>Please tick this box to declare that you have understood the notes and you have made / will make the person you are caring for / or their representative aware that there could be a change to their benefits. = {stringify(disclaimer.read)}</TextLine>
      ...
      <TextLine></TextLine>
      <TextLine>Please tick this box to confirm that you understand and make the declarations above. = {stringify(declaration.read)}</TextLine>
      <TextLine>Please tick this box if this claim form has been filled in by someone else, if so, please ensure that you understand the declarations above as another person cannot make the declarations on your behalf. = {stringify(declaration.someoneElse)}</TextLine>
      {
