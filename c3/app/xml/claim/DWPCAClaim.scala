...
object DWPCAClaim extends XMLComponent {
  ...
  def xml(claim: Claim) = {
    ...
    <DWPCAClaim>
      ...
      {question(<Employed/>,"beenEmployedSince6MonthsBeforeClaim.label",employment.beenEmployedSince6MonthsBeforeClaim, claim.dateOfClaim.fold("{CLAIM DATE - 6 months}")(dmy => (dmy - 6 months).`dd/MM/yyyy`), claimDate)}
