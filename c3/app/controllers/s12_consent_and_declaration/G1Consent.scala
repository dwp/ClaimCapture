...
object G1Consent extends Controller with CachedClaim with Navigable {
  ...
  def present = claimingWithCheck{ implicit claim => implicit request => implicit lang =>
    track(Consent) { implicit claim => Ok(views.html.s12_consent_and_declaration.g1_consent(form.fill(Consent))) }
  }
  ...
  def submit = claimingWithCheck { implicit claim => implicit request => implicit lang =>
    form.bindEncrypted.fold(
      formWithErrors => {
        ...
        BadRequest(views.html.s12_consent_and_declaration.g1_consent(formWithErrorsUpdate))},
