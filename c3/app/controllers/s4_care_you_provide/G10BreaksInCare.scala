...
object G10BreaksInCare extends Controller with CachedClaim with Navigable {
  ...
  def present = claimingWithCheck { implicit claim => implicit request => implicit lang =>
    ...
    track(BreaksInCare) { implicit claim => Ok(views.html.s4_care_you_provide.g10_breaksInCare(fillForm, breaksInCare)) }
  }
  ...
  def delete = claimingWithCheck { implicit claim => implicit request => implicit lang =>
    ...
    deleteForm.bindEncrypted.fold(
      errors    =>  BadRequest(views.html.s4_care_you_provide.g10_breaksInCare(fillForm, breaksInCare)),
      deleteForm=>  {
        val updatedBreaks = breaksInCare.delete(deleteForm.id)
        if (updatedBreaks.breaks == breaksInCare.breaks) BadRequest(views.html.s4_care_you_provide.g10_breaksInCare(fillForm, breaksInCare))
