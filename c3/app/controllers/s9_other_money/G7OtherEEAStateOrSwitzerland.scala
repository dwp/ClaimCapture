...
object G7OtherEEAStateOrSwitzerland extends Controller with CachedClaim {
  ...
  def present = claiming { implicit claim => implicit request =>
    Ok(views.html.s9_other_money.g7_otherEEAStateOrSwitzerland(form.fill(OtherEEAStateOrSwitzerland), completedQuestionGroups))
  }

  def submit = claiming { implicit claim => implicit request =>
    form.bindEncrypted.fold(
      formWithErrors => BadRequest(views.html.s9_other_money.g7_otherEEAStateOrSwitzerland(formWithErrors, completedQuestionGroups)),
