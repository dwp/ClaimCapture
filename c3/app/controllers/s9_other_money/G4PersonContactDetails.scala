...
object G4PersonContactDetails extends Controller with CachedClaim {
  ...
  def present = claiming { implicit claim => implicit request =>
    ...
    if (iAmVisible)
      Ok(views.html.s9_other_money.g4_personContactDetails(form.fill(PersonContactDetails), completedQuestionGroups))
    else
  ...
  def submit = claiming { implicit claim => implicit request =>
    form.bindEncrypted.fold(
      formWithErrors => BadRequest(views.html.s9_other_money.g4_personContactDetails(formWithErrors, completedQuestionGroups)),
