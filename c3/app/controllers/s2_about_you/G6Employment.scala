...
object G6Employment extends Controller with Routing with CachedClaim {
  ...
  def present = claiming { implicit claim => implicit request =>
    ...
    claim.questionGroup(models.domain.ClaimDate.id) match {
      case Some(n) => Ok(views.html.s2_aboutyou.g6_employment(employmentForm, completedQuestionGroups))
  ...
  def submit = claiming { implicit claim => implicit request =>
    form.bindEncrypted.fold(
      formWithErrors => BadRequest(views.html.s2_aboutyou.g6_employment(formWithErrors, completedQuestionGroups)),
