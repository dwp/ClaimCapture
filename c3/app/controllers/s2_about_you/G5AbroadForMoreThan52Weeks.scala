...
object G5AbroadForMoreThan52Weeks extends Controller with CachedClaim with Navigable {
  ...
  def present = claimingWithCheck {implicit claim =>  implicit request =>  lang =>
    ...
    track(AbroadForMoreThan52Weeks) { implicit claim => Ok(views.html.s2_about_you.g5_abroad_for_more_than_52_weeks(form.fill(AbroadForMoreThan52Weeks), trips)(lang)) }
  }
  ...
  def submit = claimingWithCheck {implicit claim =>  implicit request =>  lang =>
    ...
    form.bindEncrypted.fold(
      formWithErrors => {
        ...
        BadRequest(views.html.s2_about_you.g5_abroad_for_more_than_52_weeks(formWithErrorsUpdate, trips)(lang))
      },
