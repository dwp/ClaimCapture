...
object G2ContactDetails extends Controller with Routing with CachedClaim {
  ...
  def present = claiming { implicit claim => implicit request =>
    ...
    Ok(views.html.s2_aboutyou.g2_contactDetails(contactDetailsForm, completedQuestionGroups))
  }

  def submit = claiming { implicit claim => implicit request =>
    form.bindEncrypted.fold(
      formWithErrors => BadRequest(views.html.s2_aboutyou.g2_contactDetails(formWithErrors, completedQuestionGroups)),
