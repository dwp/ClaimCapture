...
object G3TimeOutsideUK extends Controller with Routing with CachedClaim {
  ...
  def present = claiming { implicit claim => implicit request =>
    claim.questionGroup(YourDetails.id) match {
      ...
      case _ =>
        ...
        Ok(views.html.s2_aboutyou.g3_timeOutsideUK(timeOutsideUKForm, completedQuestionGroups))
    }
  }
  ...
  def submit = claiming { implicit claim => implicit request =>
    ...
    form.bindEncrypted.fold(
      formWithErrors => BadRequest(views.html.s2_aboutyou.g3_timeOutsideUK(formWithErrors, completedQuestionGroups)),
      implicit timeOutsideUK => {
        ...
        if (timeOutsideUKFormValidated.hasErrors) BadRequest(views.html.s2_aboutyou.g3_timeOutsideUK(timeOutsideUKFormValidated, completedQuestionGroups))
