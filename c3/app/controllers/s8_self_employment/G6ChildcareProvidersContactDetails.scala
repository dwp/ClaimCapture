...
object G6ChildcareProvidersContactDetails extends Controller with CachedClaim {
  ...
  def present = claiming { implicit claim => implicit request =>
    ...
    payToLookAfterChildren  match {
      case true => whenSectionVisible(Ok(views.html.s8_self_employment.g6_childcareProvidersContactDetails(form.fill(ChildcareProvidersContactDetails), completedQuestionGroups)))
  ...
  def submit = claiming { implicit claim =>
    implicit request =>
      form.bindEncrypted.fold(
        formWithErrors => BadRequest(views.html.s8_self_employment.g6_childcareProvidersContactDetails(formWithErrors, completedQuestionGroups)),
