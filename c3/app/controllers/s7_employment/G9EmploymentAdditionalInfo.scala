...
object G9EmploymentAdditionalInfo extends Controller with CachedClaim with Navigable{
  ...
  def present = claimingWithCheck {implicit claim =>  implicit request =>  lang =>
     track(EmploymentAdditionalInfo) { implicit claim => Ok(views.html.s7_employment.g9_employmentAdditionalInfo(form.fill(EmploymentAdditionalInfo))(lang)) }
  }
  ...
  def submit = claimingWithCheck {implicit claim =>  implicit request =>  lang =>
    form.bindEncrypted.fold(
      formWithErrors => {
        ...
        BadRequest(views.html.s7_employment.g9_employmentAdditionalInfo(formWithErrorsUpdate)(lang))
      },
