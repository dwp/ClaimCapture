...
object G4LastWage extends Controller with CachedClaim {
  ...
  def present(jobID: String) = claiming { implicit claim => implicit request =>
    dispatch(Ok(views.html.s7_employment.g4_lastWage(form.fillWithJobID(LastWage, jobID), completedQuestionGroups(LastWage, jobID))))
  }

  def submit = claimingInJob { jobID => implicit claim => implicit request =>
    form.bindEncrypted.fold(
      formWithErrors => dispatch(BadRequest(views.html.s7_employment.g4_lastWage(formWithErrors, completedQuestionGroups(LastWage, jobID)))),
