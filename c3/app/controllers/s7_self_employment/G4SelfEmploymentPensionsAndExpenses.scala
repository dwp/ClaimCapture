...
import controllers.s7_self_employment.SelfEmployment._
...
object G4SelfEmploymentPensionsAndExpenses extends Controller with CachedClaim with Navigable {
  ...
  def selfEmploymentYourAccounts( lang: Lang)(implicit claim: Claim, request: Request[AnyContent]): ClaimResult = {
    track(SelfEmploymentPensionsAndExpenses) { implicit claim => Ok(views.html.s7_self_employment.g4_selfEmploymentPensionAndExpenses(form.fill(SelfEmploymentPensionsAndExpenses))(lang)) }
  }
  ...
  def submit = claimingWithCheck { implicit claim =>  implicit request =>  lang =>
    form.bindEncrypted.fold(
      formWithErrors => {
        ...
        BadRequest(views.html.s7_self_employment.g4_selfEmploymentPensionAndExpenses(formWithErrorsUpdate)(lang))
      },
