...
import controllers.s_pay_details.PayDetails._
...
object GBankBuildingSocietyDetails extends Controller with CachedClaim with Navigable {
  ...
  private def bankBuildingSocietyDetails(lang: Lang)(implicit claim: Claim, request: Request[AnyContent]): ClaimResult = {
    ...
    if (iAmVisible) track(BankBuildingSocietyDetails) { implicit claim => Ok(views.html.s_pay_details.g_bankBuildingSocietyDetails(form.fill(BankBuildingSocietyDetails))(lang)) }
  ...
  def submit = claimingWithCheck { implicit claim =>  implicit request =>  lang =>
    form.bindEncrypted.fold(
      formWithErrors => {
        ...
        BadRequest(views.html.s_pay_details.g_bankBuildingSocietyDetails(afterIgnoreGroupBy)(lang))
      },
