...
object Declaration {
  ...
  def xml(claim: Claim) = {
    ...
    <Declaration>
      ...
      <TextLine>{titleCase(consent.informationFromEmployer.answer)}</TextLine>
      {
      ...
      <TextLine>{titleCase(consent.informationFromPerson.answer)}</TextLine>
      {
      ...
      <TextLine>{titleCase(additionalInfo.welshCommunication)}</TextLine>
