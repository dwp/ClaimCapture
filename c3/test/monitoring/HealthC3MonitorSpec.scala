...
class HealthC3MonitorSpec extends Specification with MockInjector {
  ...
  "Health Monitor during tests" should {
    ...
    "must report a healthy cache" in new WithBrowser(app = FakeApplication(withGlobal = Some(global))) with PageObjects {
      val page = G1BenefitsPage(context)
      page goToThePage()
      TestHealthMonitor.runHealthChecks().get("c3-cache").get.isHealthy must beTrue
    }
    "must report a unhealthy IL3 connection" in new WithBrowser(app = FakeApplication(withGlobal = Some(global))) with PageObjects {
      val page = G1BenefitsPage(context)
      page goToThePage()
