  ...
  "Their Contact Details Form" should {
    ...
    "map data into case class" in {
      CareYouProvide.theirContactDetailsForm.bind(
        ...
        formWithErrors => formWithErrors.errors must equalTo("Valid"),
        theirContactDetails => { theirContactDetails.address.lineOne must equalTo(Some("lineOne"))
    ...
    "reject an invalid postcode" in {
      CareYouProvide.theirContactDetailsForm.bind(
        Map("address.lineOne" -> "Walkerstreet", "address.lineTwo" -> "", "address.lineThree" -> "", "postcode" -> "SE 6EH")
      ).fold(
    ...
    "reject an invalid phone number" in {
      CareYouProvide.theirContactDetailsForm.bind(
        Map("address.lineOne" -> "Walkerstreet", "address.lineTwo" -> "", "address.lineThree" -> "", "phoneNumber" -> "abcdef")
      ).fold(
