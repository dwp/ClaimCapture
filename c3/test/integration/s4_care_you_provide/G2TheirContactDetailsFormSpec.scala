  ...
  "Their Contact Details Form" should {
    ...
    "map data into case class" in {
      CareYouProvide.theirContactDetailsForm.bind(
        Map("address.lineOne" -> "lineOne", "address.lineTwo" -> "lineTwo", "address.lineThree" -> "lineThree", "postcode.content" -> validPostcode, "phoneNumber" -> "020-76542059")
      ).fold(
        ...
        theirContactDetails => {
          ...
          theirContactDetails.postcode must equalTo(Some(Postcode(Some(validPostcode))))
    ...
    "reject an invalid postcode" in {
      CareYouProvide.theirContactDetailsForm.bind(
        Map("address.lineOne" -> "lineOne", "address.lineTwo" -> "", "address.lineThree" -> "", "postcode.content" -> "1234567890")
      ).fold(
