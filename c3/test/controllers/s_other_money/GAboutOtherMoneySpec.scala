...
class GAboutOtherMoneySpec extends Specification {
  ...
  "Benefits and payments - Controller" should {
    ...
    "present 'Other money '" in new WithApplication with Claiming {
      ...
      val result = GAboutOtherMoney.present(request)
    ...
    "add submitted data to the cached claim" in new WithApplication with Claiming {
      ...
      val result = GAboutOtherMoney.submit(request)
    ...
    "return a bad request after an invalid submission" in new WithApplication {
      "reject invalid yesNo answers" in new WithApplication with Claiming {
        ...
        val result = controllers.s_other_money.GAboutOtherMoney.submit(request)
