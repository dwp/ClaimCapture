...
class GBreaksInCareSummaryNavigationIntegrationSpec extends Specification {
  ...
  "Breaks in care summary page" should {
    ...
    "navigate to hospital details page on valid submission i.e. hosptial selected" in new WithJsBrowser with PageObjects {
      val page = GBreaksInCareSummaryPage(context)
      page goToThePage()

      val claim = new TestData
      claim.BreakSummaryOtherYesNo = Mappings.yes
      claim.BreakSummaryAnswer = Breaks.hospital
      page fillPageWith claim
      val nextPage = page submitPage()

      nextPage must beAnInstanceOf[GBreaksInCareHospitalPage]
    }
    ...
    "navigate to respite details page on valid submission i.e. respite selected" in new WithJsBrowser with PageObjects {
      ...
      val claim = new TestData
      claim.BreakSummaryOtherYesNo = Mappings.yes
      claim.BreakSummaryAnswer = Breaks.carehome
      page fillPageWith claim
      val nextPage = page submitPage()

      nextPage must beAnInstanceOf[GBreaksInCareRespitePage]
    }

    "navigate to other-care details page on valid submission i.e. another selected" in new WithJsBrowser with PageObjects {
      val page = GBreaksInCareSummaryPage(context)
      page goToThePage()

      val claim = new TestData
      claim.BreakSummaryOtherYesNo = Mappings.yes
      claim.BreakSummaryAnswer = Breaks.another
      page fillPageWith claim
      val nextPage = page submitPage()

      nextPage must beAnInstanceOf[GBreaksInCareOtherPage]
    }

    "navigate to education page on valid submission - no further details" in new WithJsBrowser with PageObjects {
      val page = GBreaksInCareSummaryPage(context)
      page goToThePage()
      browser.click("#breaksummary_other_no")

      val claim = new TestData
      claim.BreakSummaryOtherYesNo = Mappings.no
      page fillPageWith claim
      val nextPage = page submitPage()
    ...
    "navigate to correct 2nd break hospital page from 3 hospital breaks when click change" in new WithJsBrowser with PageObjects {
      ...
      browser.url must contain(GBreaksInCareHospitalPage.url)
    ...
    "navigate to correct 2nd break respite page when click change" in new WithJsBrowser with PageObjects {
      ...
      browser.url must contain(GBreaksInCareRespitePage.url)
      val hospitalAdmittedYear = browser.find("#whenWereYouAdmitted_year").getValue
      hospitalAdmittedYear mustEqual "2001"
    }

    /*
        "navigate to other page when click change" in new WithJsBrowser with PageObjects {
    ...
    */
    ...
    "delete hospital 2nd break from 3 breaks when click delete and confirm yes" in new WithJsBrowser with PageObjects {
      ...
      browser.url must contain(GBreaksInCareSummaryPage.url)
    ...
    "delete other break when click delete" in new WithJsBrowser with PageObjects {
