...
class GBreaksInCareSummaryNavigationIntegrationSpec extends Specification {
  ...
  "Breaks in care summary page" should {
    ...
    "navigate to education page on valid submission - no further breaks to add" in new WithJsBrowser with PageObjects {
      val page = GBreaksInCareSummaryPage(context)
      page goToThePage()
    ...
    "navigate to other page which is 3rd is breaks list when click change" in new WithJsBrowser with PageObjects {
      ...
      browser.url must contain(GBreaksInCareOtherPage.url)
