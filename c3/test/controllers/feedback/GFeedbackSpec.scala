...
class GFeedbackSpec extends Specification {
  ...
  "Feedback page" should {
    ...
    "good submit redirect to uk gov page" in new WithApplication(app = LightFakeApplication(additionalConfiguration = Map("feedback.gb.thankyou.url" -> "/govuk/thankyou"))) with Claiming {
      val request = FakeRequest().withSession(CachedClaim.key -> claimKey).withFormUrlEncodedBody(inputSatisfiedOnlyOK: _*)
      val result = GFeedback.submit(request)
      status(result) mustEqual SEE_OTHER
    ...
    "good submit for nissa redirect to ni gov page" in new WithApplication(app = LightFakeApplication(additionalConfiguration = Map("origin.tag" -> "NI", "feedback.ni.thankyou.url" -> "/nissagov/thankyou"))) with Claiming {
      val request = FakeRequest().withSession(CachedClaim.key -> claimKey).withFormUrlEncodedBody(inputSatisfiedOnlyOK: _*)
      val result = GFeedback.submit(request)
      status(result) mustEqual SEE_OTHER
