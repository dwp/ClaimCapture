...
class GFeedbackSpec extends Specification {
  ...
  "Feedback page" should {
    ...
    "display form going to feedback submit:Claim when call Claim present()" in new WithBrowser() {
      browser.goTo("/feedback/feedback")
      browser.pageSource() must contain("feedback/feedback/Claim")
    }

    "display form going to feedback submit:Circs when call Circs present()" in new WithBrowser() {
      browser.goTo("/circumstances/feedback")
      browser.pageSource() must contain("/feedback/feedback/Circs")
    }
