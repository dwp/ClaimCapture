...
class G10BreaksInCareIntegrationSpec extends Specification with Tags {
  "Breaks in care" should {
    ...
    """present "completed" when no more breaks are required""" in new WithBrowser with Navigation with BrowserMatchers {
    ...
    "display dynamic question text if user answered that they care for this person for 35 hours or more each week before your claim date" in new WithBrowser with Navigation with BrowserMatchers {
    ...
    "display dynamic question text if user answered that they did NOT care for this person for 35 hours or more each week before your claim date" in new WithBrowser with Navigation with BrowserMatchers {
      Formulate.claimDate(browser)
      Formulate.theirPersonalDetails(browser)
      Formulate.theirContactDetails(browser)
      Formulate.moreAboutThePersonWithClaimedAllowanceBefore(browser)
    ...
    """not record the "yes/no" answer upon starting to add a new break but "cancel".""" in new WithBrowser with Navigation with BrowserMatchers {
      browser.goTo("/care-you-provide/breaks-in-care")
      titleMustEqual("Breaks in care - About the care you provide")
    ...
    """remember "no more breaks" upon stating "no more breaks" and returning to "breaks in care".""" in new WithBrowser with Navigation with BrowserMatchers {
      goTo("/care-you-provide/breaks-in-care")
      titleMustEqual("Breaks in care - About the care you provide")
