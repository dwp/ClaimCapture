...
class GYourDetailsFormSpec extends Specification {
  ...
  "Your Details Form" should {
    ...
    "allow name characters including space dash and apos" in new WithApplication(app = LightFakeApplication(additionalConfiguration = Map("surname-drs-regex" -> "true"))) {
      GYourDetails.form(FakeRequest()).bind(
        Map("title" -> title,
        ...
        f => {
          f.firstName mustEqual "Peter - Parker"
        })
    }

    "disallow name with any characters of numbers" in new WithApplication(app = LightFakeApplication(additionalConfiguration = Map("surname-drs-regex" -> "true"))) {
      GYourDetails.form(FakeRequest()).bind(
        Map("title" -> title,
          "firstName" -> "Peter The 2nd",
          "middleName" -> middleName,
          "surname" -> "O'Neill",
          "nationalInsuranceNumber.nino" -> " N R 0 1 0 2 0 3 A ",
          "nationality" -> nationality,
          ...
          "alwaysLivedUK" -> alwaysLivedUK)).fold(
        formWithErrors => {
          println("formWitErrors:" + formWithErrors.errors)
          formWithErrors.errors.length must equalTo(1)
          formWithErrors.errors(0).message must equalTo(Mappings.errorNameRestrictedCharacters)
        },
        f => {
