  ...
  "Circs Other Break" should {
    ...
    "update existing other break start date from 2001 to 2002" in new WithApplication with Claiming {
      ...
      getClaimFromCache(result, CachedChangeOfCircs.key).questionGroup(CircsBreaksInCare) must beLike {
        case Some(b: CircsBreaksInCare) => b.breaks.head.caringStarted.get.date.get.year.get shouldEqual 2001
      }
      ...
      getClaimFromCache(result, CachedChangeOfCircs.key).questionGroup(CircsBreaksInCare) must beLike {
        case Some(b: CircsBreaksInCare) => b.breaks.head.caringStarted.get.date.get.year.get shouldEqual 2002
      }
    }
  }
  section("unit", models.domain.Breaks.id)
