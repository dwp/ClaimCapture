 ...
 */
class GAddressChangeFormSpec extends Specification {
  ...
  "Report a change in your address - Address Change Form" should {
      ...
      GAddressChange.form.bind(
        Map("moreAboutChanges" -> moreAboutChanges, "newPostcode" -> postCode, "previousPostcode" -> postCode)
          ...
          formWithErrors => {
            ...
            formWithErrors.errors(0).message must equalTo("error.carerprevaddress.lines.required")
            formWithErrors.errors(1).message must equalTo(Mappings.errorRequired)
            formWithErrors.errors(2).message must equalTo("error.carernewaddress.lines.required")
          },
    ...
    "reject if second line of address is empty for new address" in new WithApplication {
      GAddressChange.form.bind(
        ...
        formWithErrors => {
          formWithErrors.errors.length must equalTo(1)
          formWithErrors.errors(0).message must equalTo("error.carernewaddress.lines.required")
        },
        f => "This mapping should not happen." must equalTo("Valid")
      )
    }
